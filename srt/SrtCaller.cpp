...
void SrtUrl::parse(const string &strUrl) {
    ...
    auto _params = FindField(url.data(), "?", NULL);
...
void SrtCaller::onConnect() {
    ...
    _socket = std::make_shared<hv::UdpClient>(_poller);
    _socket->connect(_url._port, _url._host.c_str());
    weak_ptr<SrtCaller> weak_self = shared_from_this();
    _socket->onMessage = [weak_self](const hv::SocketChannelPtr &channel, hv::Buffer *buf) mutable {
        auto strong_self = weak_self.lock();
        if (!strong_self) {
            return;
        }
        strong_self->inputSockData((uint8_t *)buf->data(), buf->size(), hio_peeraddr(channel->io()));
    };
...
void SrtCaller::sendHandshakeInduction() {
    ...
    sockaddr_u dataSenderAddr;
    sockaddr_set_ipport(&dataSenderAddr, _url._host.c_str(), _url._port);
    req->assignPeerIPBE(&dataSenderAddr.sa);
    req->storeToData();
...
void SrtCaller::sendHandshakeConclusion() {
    ...
    sockaddr_u addr;
    sockaddr_set_ipport(&addr, _url._host.c_str(), _url._port);
    req->assignPeerIPBE(&addr.sa);
...
void SrtCaller::sendPacket(Buffer::Ptr pkt, bool flush) {
    //TraceL << pkt->size();
    _socket->sendto(pkt->data(), pkt->size());
