...
static void parse_http_response(const SockException &ex, HttpResponsePtr res,
                                const function<void(const hv::Json &,const string &)> &fun){
    static hv::Json nullType;
    if (ex) {
    ...
    if (res->status_code != 200) {
        auto errStr = StrPrinter << "[bad http status code]:" << res->status_code << endl;
        fun(nullType, errStr);
        return;
    }
    
    if (res->GetInt("code") != 0) {
        auto errStr = StrPrinter << "[json code]:" << "code=" << res->GetInt("code") << ",msg=" << res->GetString("msg") << endl;
        fun(nullType, errStr);
        return;
    }
    try {
...
string to_string(const hv::Json &value){
    return value.dump();
}
...
const char *getContentType(const hv::Json &value){
...
string getVhost(const hv::Json &value) {
    const char *key = VHOST_KEY;
    auto val = value.find(key);
    return val != value.end() ? val->get<std::string>() : "";
}
...
void do_http_hook(const string &url, const ArgsType &body, const function<void(const hv::Json &, const string &)> &func, uint32_t retry) {
    ...
    req->SetMethod("POST");
    req->Set("mediaServerId", mediaServerId);
    auto vhost = getVhost(body);
    if (!vhost.empty()) {
        req->SetHeader("X-VHOST", vhost);
    }
    req->retry_count = retry;
    req->retry_delay = retry_delay;
    req->timeout = hook_timeoutSec;

    Ticker ticker;
    requests::async(req, [ticker, url, func](HttpResponsePtr res){
        SockException err;
        parse_http_response(err, res, [&](const hv::Json &obj, const string &err) {
            if (!err.empty()) {
                // hook失败
                WarnL << "hook " << url << " " << ticker.elapsedTime() << "ms,failed" << err;
            } else if (ticker.elapsedTime() > 500) {
                //hook成功，但是hook响应超过500ms，打印警告日志
                DebugL << "hook " << url << " " << ticker.elapsedTime() << "ms,success";
            }
...
void do_http_hook(const string &url, const ArgsType &body, const function<void(const hv::Json &, const string &)> &func) {
...
static void reportServerKeepalive() {
    ...
    g_keepalive_timer = std::make_shared<Timer>(alive_interval, []() {
        getStatisticJson([](const hv::Json &data) mutable {
            ArgsType body;
            body["data"] = data;
            //执行hook
...
static mINI jsonToMini(const hv::Json &obj) {
    mINI ret;
    if (obj.is_object()) {
        for (auto& el : obj.items())
        {
            try {
                ...
                WarnL << "Json is not convertible to string, key: " << el.key() << ", value : " << el.value();
            }
        }
    }
    return ret;
...
void installWebHook(){
    ...
    NoticeCenter::Instance().addListener(&web_hook_tag, Broadcast::kBroadcastMediaPublish, [](BroadcastMediaPublishArgs) {
        ...
        //执行hook
        do_http_hook(hook_publish, body, [invoker](const hv::Json &obj, const string &err) mutable {
            if (err.empty()) {
                //推流鉴权成功
    ...
    NoticeCenter::Instance().addListener(&web_hook_tag,Broadcast::kBroadcastMediaPlayed,[](BroadcastMediaPlayedArgs){
        ...
        // body["id"] = sender.getIdentifier();
        //执行hook
        do_http_hook(hook_play,body,[invoker](const hv::Json &obj,const string &err){
            invoker(err);
        });
    });
    ...
    NoticeCenter::Instance().addListener(&web_hook_tag,Broadcast::kBroadcastOnGetRtspRealm,[](BroadcastOnGetRtspRealmArgs){
        ...
        // body["id"] = sender.getIdentifier();
        //执行hook
        do_http_hook(hook_rtsp_realm,body, [invoker](const hv::Json &obj,const string &err){
            if(!err.empty()){
    ...
    NoticeCenter::Instance().addListener(&web_hook_tag,Broadcast::kBroadcastOnRtspAuth,[](BroadcastOnRtspAuthArgs){
        ...
        body["realm"] = realm;
        //执行hook
        do_http_hook(hook_rtsp_auth,body, [invoker](const hv::Json &obj,const string &err){
            if(!err.empty()){
                //认证失败
                invoker(false,makeRandStr(12));
                return;
            }
            invoker(obj["encrypted"], obj["passwd"]);
        });
    });
    ...
    NoticeCenter::Instance().addListener(&web_hook_tag, Broadcast::kBroadcastNotFoundStream, [](BroadcastNotFoundStreamArgs) {
        ...
        // Hook回复立即关闭流
        auto res_cb = [closePlayer](const hv::Json &res, const string &err) {
            bool flag = res["close"];
            if (flag) {
                closePlayer();
            }
    ...
    NoticeCenter::Instance().addListener(&web_hook_tag,Broadcast::kBroadcastShellLogin,[](BroadcastShellLoginArgs){
        ...
        //执行hook
        do_http_hook(hook_shell_login,body, [invoker](const hv::Json &,const string &err){
            invoker(err);
        });
    });
    ...
    NoticeCenter::Instance().addListener(&web_hook_tag,Broadcast::kBroadcastStreamNoneReader,[](BroadcastStreamNoneReaderArgs) {
        ...
        //执行hook
        do_http_hook(hook_stream_none_reader,body, [weakSrc](const hv::Json &obj,const string &err){
            bool flag = obj["close"].get<bool>();
    ...
    NoticeCenter::Instance().addListener(&web_hook_tag,Broadcast::kBroadcastHttpAccess, [](BroadcastHttpAccessArgs){
        ...
        //执行hook
        do_http_hook(hook_http_access,body, [invoker](const hv::Json &obj,const string &err){
            if(!err.empty()){
            ...
            invoker(obj["err"],obj["path"],obj["second"]);
        });
    });
#endif
