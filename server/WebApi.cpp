...
using HttpApi = std::function<void(const Parser &parser, const HttpSession::HttpResponseInvoker &invoker, SockInfo &sender)>;
//http api列表
static std::map<string, HttpApi> s_map_api;
...
static HttpApi toApi(const std::function<void(API_ARGS_MAP_ASYNC)> &cb) {
...
static HttpApi toApi(const std::function<void(API_ARGS_MAP)> &cb) {
...
static HttpApi toApi(const std::function<void(API_ARGS_JSON_ASYNC)> &cb) {
    return [cb](const Parser &parser, const HttpSession::HttpResponseInvoker &invoker, SockInfo &sender) {
        GET_CONFIG(string, charSet, Http::kCharSet);
        HttpSession::KeyValue headerOut;
        headerOut["Content-Type"] = string("application/json; charset=") + charSet;
...
static HttpApi toApi(const std::function<void(API_ARGS_JSON)> &cb) {
...
static HttpApi toApi(const std::function<void(API_ARGS_STRING_ASYNC)> &cb) {
    return [cb](const Parser &parser, const HttpSession::HttpResponseInvoker &invoker, SockInfo &sender) {
        GET_CONFIG(string, charSet, Http::kCharSet);
        HttpSession::KeyValue headerOut;
        headerOut["Content-Type"] = string("application/json; charset=") + charSet;
...
static HttpApi toApi(const std::function<void(API_ARGS_STRING)> &cb) {
...
void api_regist(const string &api_path, const std::function<void(API_ARGS_MAP)> &func) {
    s_map_api.emplace(api_path, toApi(func));
}

void api_regist(const string &api_path, const std::function<void(API_ARGS_MAP_ASYNC)> &func) {
    s_map_api.emplace(api_path, toApi(func));
}

void api_regist(const string &api_path, const std::function<void(API_ARGS_JSON)> &func) {
    s_map_api.emplace(api_path, toApi(func));
}

void api_regist(const string &api_path, const std::function<void(API_ARGS_JSON_ASYNC)> &func) {
    s_map_api.emplace(api_path, toApi(func));
}

void api_regist(const string &api_path, const std::function<void(API_ARGS_STRING)> &func){
    s_map_api.emplace(api_path, toApi(func));
}

void api_regist(const string &api_path, const std::function<void(API_ARGS_STRING_ASYNC)> &func){
    s_map_api.emplace(api_path, toApi(func));
}
...
static ApiArgsType getAllArgs(const Parser &parser) {
    ...
    if (parser["Content-Type"].find("application/x-www-form-urlencoded") == 0) {
        ...
        try {
            std::stringstream ss(parser.Content());
            Value jsonArgs;
            ss >> jsonArgs;
...
//FFmpeg拉流代理器列表
static std::unordered_map<string, FFmpegSource::Ptr> s_ffmpegMap;
static std::recursive_mutex s_ffmpegMapMtx;

#if defined(ENABLE_RTPPROXY)
//rtp服务器列表
static std::unordered_map<string, RtpServer::Ptr> s_rtpServerMap;
...
void SockInfoToJson(Value& val, SockInfo* info) {
...
void installWebApi() {
    ...
    api_regist("/index/api/getThreadsLoad", [](API_ARGS_MAP_ASYNC){
        EventPollerPool::Instance().getExecutorDelay([invoker, headerOut](const std::vector<int> &vecDelay) {
            Value val;
    ...
    api_regist("/index/api/getWorkThreadsLoad", [](API_ARGS_MAP_ASYNC){
        WorkThreadPool::Instance().getExecutorDelay([invoker, headerOut](const std::vector<int> &vecDelay) {
            Value val;
    ...
    api_regist("/index/api/getAllSession", [](API_ARGS_MAP){
        ...
        SessionMap::Instance().for_each_session([&](const string &id, const Session::Ptr &session){
            ...
            SockInfoToJson(jsession, session.get());
            jsession["id"] = id;
    ...
    api_regist("/index/api/kick_sessions",[](API_ARGS_MAP){
        ...
        std::list<Session::Ptr> session_list;
        SessionMap::Instance().for_each_session([&](const string &id,const Session::Ptr &session){
            if(local_port != 0 && local_port != session->get_local_port()){
                return;
            }
    ...
    api_regist("/index/api/getSnap", [](API_ARGS_MAP_ASYNC){
        ...
        FFmpegSnap::makeSnap(allArgs["url"], new_snap_tmp, allArgs["timeout_sec"], 
