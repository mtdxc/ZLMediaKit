...
void WebRTCUrl::parse(const string &strUrl, bool isPlayer) {
    ...
    if (it != kv.end()) {
        _signaling_protocols = (SignalingProtocols)(stoi(it->second));
    }

    char url_buf[512];
    snprintf(url_buf, sizeof(url_buf), "%s://%s:%d/index/api/%s?app=%s&stream=%s", 
        _is_ssl ? "https" : "http", _host.data(), _port, 
        isPlayer ? "whep" : "whip", app.data(), stream.data());
    _negotiate_url = url_buf;
}
...
void WebRtcClient::connectivityCheck() {
    DebugL;
    _transport->connectivityCheckForSFU();
}
...
void WebRtcClient::onNegotiateFinish() {
    ...
    switch (_url._signaling_protocols) {
        case SignalingProtocols::WHEP_WHIP:
            //SFU模式不会存在IP不通的情况，answer中就携带了candidates, 直接进行connectiviryCheck
            connectivityCheck();
            break;
        case SignalingProtocols::WEBSOCKET: 
            //P2P模式需要gathering candidates
            gatheringCandidate(_peer->getIceServer());
            break;
        default: 
            throw std::invalid_argument(StrPrinter << "not support signaling_protocols: " << (int)_url._signaling_protocols);
    }
}
...
void WebRtcClient::doNegotiate() {
    DebugL;
    switch (_url._signaling_protocols) {
        case SignalingProtocols::WHEP_WHIP: return doNegotiateWhepOrWhip();
        case SignalingProtocols::WEBSOCKET: return doNegotiateWebsocket();
...
void WebRtcClient::doNegotiateWebsocket() {
    ...
    InfoL << "register to signaling server " << _url._host << ":" << _url._port << " with id " << room_id;
...
void WebRtcClient::checkIn() {
    DebugL;
    weak_ptr<WebRtcClient> weak_self = static_pointer_cast<WebRtcClient>(shared_from_this());
    _peer->checkIn(_url._peer_room_id, _url, _transport->getIdentifier(), _transport->createOfferSdp(), isPlayer(),
...
void WebRtcClient::gatheringCandidate(IceServerInfo::Ptr ice_server) {
    ...
    _transport->gatheringCandidate(ice_server, [weak_self](const std::string& transport_identifier, 
...
void WebRtcClient::doBye() {
    ...
    switch (_url._signaling_protocols) {
        case SignalingProtocols::WHEP_WHIP: return doByeWhepOrWhip();
        case SignalingProtocols::WEBSOCKET: return checkOut();
        default: throw std::invalid_argument(StrPrinter << "not support signaling_protocols: " << (int)_url._signaling_protocols);
    }
