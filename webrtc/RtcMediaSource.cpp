...
bool needTransToOpus(CodecId codec) {
  return codec == CodecAAC;// || codec == CodecG711A || codec == CodecG711U;
}

bool needTransToAac(CodecId codec) {
  return codec == CodecOpus;// || codec == CodecG711A || codec == CodecG711U;
}
  ...
  : RtspMediaSourceMuxer(vhost, strApp, strId, option, title, RTC_SCHEMA)
{
  ...
  _transcode = option.transcode_rtc_audio;
#else
  if (option.transcode_rtc_audio) {
...
#if defined(ENABLE_FFMPEG)
    if (_transcode && needTransToOpus(frame->getCodecId())) {
...
bool RtcMediaSourceMuxer::addTrack(const Track::Ptr & track)
{
  ...
  if (_transcode && needTransToOpus(track->getCodecId())) {
...
bool RtcMediaSourceImp::addTrack(const Track::Ptr &track)
{
  if (_muxer) {
    Track::Ptr newTrack = track;
    if (_option.transcode_rtc_audio && needTransToAac(track->getCodecId())) {
