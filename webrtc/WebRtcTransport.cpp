...
const char *sockTypeStr(SocketHelper::Ptr sock) {
    if (sock) {
        switch (sock->getSock()->sockType()) {
...
void WebRtcTransport::onCreate() {
    ...
    auto role = IceAgent::Role::Controlling;
    auto implementation = IceAgent::Implementation::Full;
    if (_role == Role::PEER) {
...
void WebRtcTransport::inputSockData(char *buf, int len, IceTransport::Pair::Ptr tuple) {
    // DebugL;
    _recv_ticker.resetTime();
    if (_ice_agent->processSocketData((const uint8_t *)buf, len, tuple)) {
        return;
    }
