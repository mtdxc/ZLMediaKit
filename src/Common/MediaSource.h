...
public:
    ...
    */
    ...
    // 获取所在线程, 此函数一般强制重载
    virtual toolkit::EventPollerPtr getOwnerPoller(MediaSource &sender) { throw NotImplemented(toolkit::demangle(typeid(*this).name()) + "::getOwnerPoller not implemented"); }
...
private:
    std::shared_ptr<toolkit::Timer> _async_close_timer;
};
...
#define GET_OPT_VALUE(key) getArgsValue(allArgs, #key, key)
        ...
        GET_OPT_VALUE(transcode_rtc_audio);
...
public:
    ...
    // 获取所在线程
    toolkit::EventPollerPtr getOwnerPoller();
    ...
    // 异步查找流
    static void findAsync(const MediaInfo &info, const std::shared_ptr<hv::SocketChannel> &session, const std::function<void(const Ptr &src)> &cb);
    // 遍历所有流
...
private:
    ...
    std::weak_ptr<MediaSourceEvent> _listener;
    toolkit::EventPollerPtr _default_poller;
    // 对象个数统计
