...
private:
    ...
    void flushGop() {
        ...
        for (auto& lst : _storage->getCache()) {
            for(const std::pair<bool, T> &pr : lst)
...
private:
    ...
    std::shared_ptr<RingReader> attach(const hv::EventLoopPtr &poller, bool use_cache) {
    ...
    std::shared_ptr<RingReader> attach(const hv::EventLoopPtr &poller, bool use_cache = true) {
        typename RingReaderDispatcher::Ptr dispatcher;
        {
            ...
            if (!ref) {
                ...
                auto onDealloc = [poller](RingReaderDispatcher *ptr) { poller->runInLoop([ptr]() { delete ptr; }); };
...
private:
    void onSizeChanged(const hv::EventLoopPtr &poller, int size, bool add_flag) {
        if (size == 0) {
            LOCK_GUARD(_mtx_map);
...
private:
    struct HashOfPtr {
        std::size_t operator()(const hv::EventLoopPtr &key) const { return (std::size_t)key.get(); }
    };
...
private:
    ...
    std::unordered_map<hv::EventLoopPtr, typename RingReaderDispatcher::Ptr, HashOfPtr> _dispatcher_map;
};
