       ...
       || ts_header_info._adaptation_field_control == 3 ) {
        ...
        if(ts_header_info._PID == 0x00){
            ...
            if (_pat._table_id != 0x00) {
                hloge("pat table id(0x%02x) error, it must be 0x00", _pat._table_id);
                return -1;
            }
            // PAT = section_length + 3
            if((188 - npos) <= (_pat._section_length + 3)) {
                hloge("pat _section_length(%d) error, the left len:%d", _pat._section_length, (188 - npos));
                return -1;
            }
            pos++;
            _pat._pid_vec.clear();
...
//               CRC_32 use pat to calc crc32, eq
            ...
            if (_pmt._table_id != 0x02) {
                hloge("pmt tableid(0x%02x) error, it must be 0x02", _pmt._table_id);
                return -1;
            }
...
int ts_demux::decode(SRT_DATA_MSG_PTR data_ptr, TS_DATA_CALLBACK_PTR callback)
{
    ...
    if (!data_ptr || (data_ptr->size() < 188) || (data_ptr->data()[0]%188 != 0))
    {
        return -1;
    }

    unsigned int count = data_ptr->size()/188;
    //path = data_ptr->get_path();
    for (unsigned int index = 0; index < count; index++)
    {
        unsigned char* data = (unsigned char*)data_ptr->data() + 188*index;
        if (data[0] != 0x47) {
            continue;
        }
...
void ts_demux::insert_into_databuf(unsigned char* data_p, size_t data_size, std::string key_path, unsigned short pid) {
    ...
    buf->assign((const char*)data_p, data_size);
    _data_buffer_vec.push_back(buf);
    return;
}
...
int ts_demux::on_callback(TS_DATA_CALLBACK_PTR callback, unsigned short pid, std::string key_path,
    ...
    auto total_data_ptr = toolkit::BufferRaw::create();// _data_total, key_path);
    total_data_ptr->setSize(_data_total);
    size_t pos = 0;

    for (size_t index = 0; index < _data_buffer_vec.size(); index++) {
        memcpy(total_data_ptr->data() + pos, 
            _data_buffer_vec[index]->data(),
            _data_buffer_vec[index]->size());
        pos += _data_buffer_vec[index]->size();
    }
...
int ts_demux::pes_parse(unsigned char* p, size_t npos, 
    ...
    if (0x00000001 != packet_start_code_prefix) {
        hloge("pes packet start code prefix(%06x) error, it must be 0x00 00 01", packet_start_code_prefix);
        return 255;
    }
    ...
    {
        if (0x80 != (p[pos] & 0xc0)) {
            hloge("the first 2 bits:0x%02x error, it must be 0x80.", (p[pos] & 0xc0));
            return 255;
        }
