...
static std::unordered_map<string, std::weak_ptr<RtspSession> > g_mapGetter;
//对g_mapGetter上锁保护
...
void RtspSession::onError(const SockException &err) {
    ...
    if (_http_x_sessioncookie.size() != 0) {
        //移除http getter的弱引用记录
        std::lock_guard<std::recursive_mutex> lock(g_mtxGetter);
...
void RtspSession::onWholeRtspPacket(Parser &parser) {
    ...
    static std::unordered_map<string, rtsp_request_handler> s_cmd_functions;
...
void RtspSession::handleReq_ANNOUNCE(const Parser &parser) {
    ...
    auto onRes = [this, parser, full_url](const string &err, const ProtocolOption &option) {
        ...
        if (push_failed) {
            ...
            string err = StrPrinter << "ANNOUNCE:" << "Already publishing:" << _media_info.shortUrl();
    ...
    std::weak_ptr<RtspSession> weakSelf = std::dynamic_pointer_cast<RtspSession>(shared_from_this());
...
void RtspSession::emitOnPlay(){
    ...
    auto onRes = [weakSelf](const string &err) {
        ...
        if (!err.empty()) {
            ...
            strongSelf->shutdown(SockException(Err_shutdown, "401 Unauthorized:" + err));
        }
        else
...
void RtspSession::onAuthSuccess() {
    ...
    MediaSource::findAsync(_media_info, weakSelf.lock(), [weakSelf](const MediaSource::Ptr &src){
        ...
        if (!rtsp_src) {
            //未找到相应的MediaSource
            string err = StrPrinter << "no such stream:" << strongSelf->_media_info.shortUrl();
...
void RtspSession::onAuthFailed(const string &realm,const string &why,bool close) {
    ...
    if (!authBasic) {
        ...
        sendRtspResponse("401 Unauthorized",
                         {"WWW-Authenticate", StrPrinter << "Digest realm=\"" << realm << "\",nonce=\"" << _auth_nonce << "\"" });
    } else {
        //当然我们也支持base64认证,但是我们不建议这样做
        sendRtspResponse("401 Unauthorized",
                         {"WWW-Authenticate", StrPrinter << "Basic realm=\"" << realm << "\"" });
    }
    if(close) {
        shutdown(SockException(Err_shutdown, "401 Unauthorized:" + why));
    }
}
...
void RtspSession::handleReq_Play(const Parser &parser) {
    ...
    //已存在Range时不覆盖
    res_header.emplace("Range", StrPrinter << "npt=" << std::setiosflags(ios::fixed) << std::setprecision(2) << play_src->getTimeStamp(TrackInvalid) / 1000.0);
...
void RtspSession::handleReq_Get(const Parser &parser) {
    ...
    std::lock_guard<std::recursive_mutex> lock(g_mtxGetter);
    g_mapGetter[_http_x_sessioncookie] = std::dynamic_pointer_cast<RtspSession>(shared_from_this());
}
...
void RtspSession::onRcvPeerUdpData(int interleaved, const Buffer::Ptr &buf, const struct sockaddr_storage &addr) {
    ...
    if (interleaved % 2 == 0) {
        ...
        if (!_udp_connected_flags.count(interleaved)) {
            _udp_connected_flags.emplace(interleaved);
            _rtcp_socks[index]->bindPeerAddr((struct sockaddr *)&addr);
        }
...
void RtspSession::sendRtcpPacket(int track_idx, toolkit::Buffer::Ptr ptr) {
    if (_rtp_type == Rtsp::RTP_TCP) {
        ...
        _rtcp_socks[track_idx]->send(std::move(ptr));
    }
}
