...
FFmpegDecoder::FFmpegDecoder(const Track::Ptr &track, int thread_num, const std::vector<std::string> &codec_name) {
    ...
    while (true) {
        ...
        if (ret >= 0) {
            ...
            // Success
            InfoL << "打开解码器成功:" << codec->name << ", " << track->getInfo();
            break;
        }
...
std::tuple<bool, std::string> FFmpegUtils::saveFrame(const FFmpegFrame::Ptr &frame, const char *filename, AVPixelFormat fmt, int w, int h, const char *font_path) {
    ...
    jpeg_codec_ctx->width = (w > 0 && w < 8192) ? w : frame->width;
    jpeg_codec_ctx->height = (h > 0 && h < 4320) ? h : frame->height;
...
FFmpegEncoder::FFmpegEncoder(const Track::Ptr &cfg, int thread_num, int format, const std::vector<std::string> &codec_name) {
    ...
    while (true) {
        ...
        if (ret >= 0) {
            //成功
            InfoL << "打开编码器成功:" << codec->name << ", " << cfg->getInfo();
            break;
        }
        if (codec_default && codec_default != codec) {
            //硬件编解码器打开失败，尝试软件的
...
void FFmpegEncoder::inputFrame_l(FFmpegFrame::Ptr frame_in) {
    ...
    // TraceL << "input " << frame_in->pts << ", samples=" << frame_in->nb_samples;
...
bool FFmpegEncoder::encodeFrame(AVFrame *frame) {
    // TraceL << "enc " << frame->pts << " samples=" << frame->nb_samples;
...
toolkit::Buffer::Ptr FFmpegEncoder::getExtraData() const {
    if (!_context || !_context->extradata || !_context->extradata_size) {
        return nullptr;
    }
    auto ret = toolkit::BufferRaw::create(_context->extradata_size);
    ret->assign((char *)_context->extradata, _context->extradata_size);
    return ret;
}
