...
void Track::setupEncoder(int arg1, int arg2, int arg3, int bitrate) {
    ...
    auto encoder = std::make_shared<FFmpegEncoder>(cfg, 1);
    if (auto data = encoder->getExtraData()) {
       setExtraData((const uint8_t*)data->data(), data->size());
    }
    _enc_cfg = cfg;
}
#endif
...
Track::Ptr Track::getTransodeTrack(CodecId id, int arg1, int arg2, int bitrate) {
    ...
    if (!ret) {
        ...
        if (ret) {
            // 这边索引要保持唯一，MultiMediaSourceMuxer会将原始Track的帧分发到对应的RtcMediaSourceMuxer中，会导致同一类型的包被发两次
            static int transIdx = TrackMax;
